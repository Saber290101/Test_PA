      ...
      _export("ResizeCam", ResizeCam = (_dec = ccclass('ResizeCam'), _dec2 = property(Camera), _dec3 = property(Label), _dec4 = property({
        ...
        constructor() {
          ...
          this.applyResize = () => {
            ...
            var refAspect = isLandscape ? this.refLandscapeAspect : this.refPortraitAspect;
            var picked = this.pickProfile(aspect, isLandscape);
            var k = isLandscape ? picked.kLandscape : picked.kPortrait;
            var newOrthoHeight = k * (refAspect / aspect);
        ...
        pickProfile(aspect, isLandscape) {
          ...
          for (var p of this.DEVICE_PROFILES) {
            var _p$epsilon;

            var target = isLandscape ? p.aspectLandscape : p.aspectPortrait;
            var diff = Math.abs(aspect - target);
            var eps = (_p$epsilon = p.epsilon) != null ? _p$epsilon : this.epsilonDefault;
