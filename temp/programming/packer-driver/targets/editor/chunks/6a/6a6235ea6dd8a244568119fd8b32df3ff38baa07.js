System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, Vec3, Tween, director, Collider, BusState, BusController, _dec, _class, _crd, ccclass, property, PathFollower;
  ...
  return {
    ...
    execute: function () {
      ...
      _cclegacy._RF.push({}, "618ebG4L/9OT5L16d4oKEHK", "PathFollower", undefined);

      __checkObsolete__(['_decorator', 'Component', 'Vec3', 'tween', 'easing', 'Tween', 'director', 'Collider']);
      ...
      _export("PathFollower", PathFollower = (_dec = ccclass('PathFollower'), _dec(_class = class PathFollower extends Component {
        ...
        /** Chỉ số waypoint hiện tại trong path */
        ...
        /** Điểm cuối (đích) của path hiện tại */
        ...
        get distanceToDestination() {
          ...
          if (dest) {
            return Vec3.distance(this.node.worldPosition, dest);
          }
        ...
        tempStop(duration) {
          ...
          const busCtrl = this.node.getComponent(_crd && BusController === void 0 ? (_reportPossibleCrUseOfBusController({
            error: Error()
        ...
        _isPathBlocked() {
          ...
          if (this._busCacheTime <= 0 || !this._cachedBuses) {
            this._cachedBuses = scene.getComponentsInChildren(_crd && BusController === void 0 ? (_reportPossibleCrUseOfBusController({
              error: Error()
            }), BusController) : BusController);
          ...
          const myBus = this.node.getComponent(_crd && BusController === void 0 ? (_reportPossibleCrUseOfBusController({
            error: Error()
          }), BusController) : BusController);
          ...
          for (const other of this._cachedBuses) {
            ...
            const distCenterToCenter = Vec3.distance(myPos, otherPos);
            const minCenterDist = myHalfLen + otherHalfLen;
        ...
        _distToBusBody(point, busPos, busRotY, halfLen) {
          ...
          const a = new Vec3();
          const b = new Vec3(); // Thu gọn segment 1 chút (0.1m) ở hai đầu để tính toán capsule mượt mà hơn
        ...
        _onCollisionEnter(event) {
          ...
          const otherBus = otherNode.getComponent(_crd && BusController === void 0 ? (_reportPossibleCrUseOfBusController({
            error: Error()
          }), BusController) : BusController) || otherNode.getComponentInParent(_crd && BusController === void 0 ? (_reportPossibleCrUseOfBusController({
            error: Error()
          }), BusController) : BusController);
          if (!otherBus) return;
          ...
          console.log(`[Collision] Va chạm vật lý giữa 2 xe: ${this.node.name} và ${otherBus.node.name}. Khoảng cách: ${distBetween.toFixed(2)}m`); // Bỏ qua logic so sánh distanceToDestination vì nó gây lỗi khi xe có điểm đến khác nhau.
...
//# sourceMappingURL=6a6235ea6dd8a244568119fd8b32df3ff38baa07.js.map